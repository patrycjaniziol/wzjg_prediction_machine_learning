...
df['bilirubin (total)'] = pd.to_numeric(
    df['bilirubin (total)'], errors='coerce')
df['ultra-sensitive CRP'] = pd.to_numeric(
    df['ultra-sensitive CRP'], errors='coerce')
...
df['neutrophils #'] = pd.to_numeric(df['neutrophils #'], errors='coerce')
df['immature granulocytes #'] = pd.to_numeric(
    df['immature granulocytes #'], errors='coerce')
...
gridlg = GridSearchCV(LogisticRegression(), param_grid,
